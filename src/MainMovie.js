import ppk from "./images/bg.png";
import ppkLogo from "./images/ppk.png";
import "./styles/mainmovie.css";
import { AiFillPlayCircle, AiFillHeart } from "react-icons/ai";
import ppkvideo from "./videos/ppk.mp4";
...
function MainMovie({ setfavoriteMovie, favoriteMovie }) {
  ...
  function favoriteClick(e) {
    const isDuplicate = favoriteMovie.some(
      (data) => Number(e.id) === Number(data.id)
    );
  ...
  return (
    <>
      <div className="main-movie">
        {movieChck ? (
          <img src={ppk} alt="" className="main-movie-img" />
        ) : (
          <video
            src={ppkvideo}
        ...
        <div className="main-movie-content">
          <img src={ppkLogo} alt="" className="main-movie-content-img" />
          <h2>
            Watch in Telugu,Tamil, Hindi, Malayalam,
            <br /> Kannada
          </h2>
          <p>
            Worlds and families collide when the son of a prominent atheist
            falls head over heels for the daughter of his father’s nemesis, a
            revered Hindu leader.
          </p>
          <button onClick={movieChange}>
                  ...
                  <>
                    <Link to={"/playmovies/" + movie.id}>
                      <img
                        src={movie.movieimg}
                        alt={movie.moveiname}
                        className="trending-img-div"
                      />
                    </Link>
                    <AiFillHeart
                      className="trending-img-div-heart"
